...
namespace BisellsAPI.Controllers
{
    ...
    {
        [HttpPost]
        public HttpResponseMessage Save(PurchaseReturnRegister pr)
        {
            if (pr.ID > 0)
            {
        ...
        {
            return Request.CreateResponse(HttpStatusCode.OK, PurchaseReturnRegister.GetDetails(Id,LocationId));
        }
        ...
        {
            return Request.CreateResponse(HttpStatusCode.OK, PurchaseReturnRegister.GetDetails(LocationId, SupplierId, from, to));
        }
        [HttpPost]
        ...
        {
            return Request.CreateResponse(HttpStatusCode.OK, await Task.Run(() => PurchaseReturnRegister.SendMail(purchaseid, toAddress, userId, url)));
        }
        [HttpPost]
        [DeflateCompression]
