...
{
    public class SalesQuoteController : ApiController
    {
        [HttpPost]
        public HttpResponseMessage Save(Entities.Register.SalesQuoteRegister Sq)
        {
            if (Sq.ID > 0)
            {
        ...
        {
            return Request.CreateResponse(HttpStatusCode.OK, SalesQuoteRegister.GetDetails(LocationId,CustomerId,from,to));
        }

        [HttpPost]
        public HttpResponseMessage Get([FromUri] int Id,[FromBody]int LocationId)
        {
            return Request.CreateResponse(HttpStatusCode.OK, SalesQuoteRegister.GetDetails(Id,LocationId));
        }
        [HttpPost]
        [DeflateCompression]
        ...
        {
            List<SalesQuoteRegister> reg = SalesQuoteRegister.GetDetailsForConfirm(LocationId).ToList();
        ...
        {
            return Request.CreateResponse(HttpStatusCode.OK, new SalesQuoteRegister(Id, user_ID).Confirm());
        }
        [HttpPost]
        public HttpResponseMessage ToggleConfirm([FromUri] int Id, [FromBody] int user_ID)
        {
            return Request.CreateResponse(HttpStatusCode.OK, new SalesQuoteRegister(Id, user_ID).ToggleConfirm());
        }
        [HttpDelete]
        public HttpResponseMessage Delete([FromUri] int Id, [FromBody] int modifiedBy)
        {
            SalesQuoteRegister pr = new SalesQuoteRegister();
            pr.ID = Id;
        ...
        {
            return Request.CreateResponse(HttpStatusCode.OK, await Task.Run(() => SalesQuoteRegister.SendMail(salesId, toAddress, userId, url)));
        }
