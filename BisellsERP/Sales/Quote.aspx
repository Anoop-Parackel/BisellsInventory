<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Quote.aspx.cs" Inherits="BisellsERP.Sales.Quote" ValidateRequest="false" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="childContent" runat="server">
   ...
   <%-- <asp:HiddenField runat="server" Value="0" ID="hdId" ClientIDMode="Static" />--%>
    ...
    <div class="row p-b-10">
        <div class="col-sm-2">
            <h3 class="page-title m-t-0">Sales Order</h3>
        </div>
        <div class="col-sm-10">
            <div class="btn-toolbar pull-right" role="group">
                <button type="button" accesskey="v" id="btnFind" data-toggle="tooltip" data-placement="bottom" title="View previous Orders" class="btn btn-default waves-effect waves-light"><i class="ion-search"></i></button>
                ...
                <button type="button" accesskey="s" id="btnSave" data-toggle="tooltip" data-placement="bottom" title="Save the current order" class="btn btn-default waves-effect waves-light "><i class="ion-archive"></i>&nbsp;Save</button>
                <button id="btnSavePrint" accesskey="a" type="button" data-toggle="tooltip" data-placement="bottom" title="Save & Print the current order" class="btn btn-default waves-effect waves-light"><i class="ion ion-printer"></i>&nbsp;Save & Print</button>
                <button type="button" id="btnConfirm" disabled="disabled" data-toggle="tooltip" data-placement="bottom" title="Confirm sales order" class="btn btn-default waves-effect waves-light "><i class="ion-checkmark-circled"></i>&nbsp;Confirm</button>
                    ...
                    <div class="panel b-r-8">
                        <div class="panel-body">
                            ...
                            <div class="col-sm-3">
                                ...
                                <input type="text" autocomplete="off" id="txtQuantity" class="form-control" placeholder="Qty" />
                            </div>
                            <div class="col-sm-2">
...
<%--                            <asp:TextBox ID="txtNarration" CssClass="w-100" TextMode="MultiLine" Rows="3" ClientIDMode="Static" runat="server"></asp:TextBox>--%>
                            <input type="text" id="txtNarration" class="w-100" />
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Previous Sales Orders &nbsp;<a id="findFilter"><i class="fa fa-align-justify"></i></a></h4>
    ...
    <script>
        ...
        $(document).ready(function () {
          ...
          .on('changeDate', function (ev) {
            ...
            //changed address loads in fields
                ...
                if (tempItem.ItemID != '' & tempItem.ItemID != null & tempItem.ItemID != undefined & qty != '0' & qty != '' & qty != null & !isNaN(qty)) {
                    ...
                    if ((!IsNegBillingAllowed && parseFloat(tempItem.Stock) < qty) && TrackInventory == 'true') {//New Function with track inventory check
                        ...
                        if (itemExists) {
                            ...
                            html = '';
                            html += '<tr><td style="display:none">' + tempItem.ItemID + '</td><td style="display:none">0</td><td><b>' + tempItem.Name + '</b><br/><i contenteditable="true" spellcheck="false">' + description + '</i></td><td>' + tempItem.ItemCode + '</td><td style="text-align:right">' + tempItem.TaxPercentage + '</td><td style="text-align:right">' + tempItem.MRP + '</td><td contenteditable="true" class="numberonly" style="text-align:right">' + tempItem.SellingPrice + '</td><td style="text-align:right">0</td><td style="text-align:right" contenteditable="true" class="numberonly">  ' + qty + ' </td><td style="text-align:right">' + TaxAmount + '</td><td style="text-align:right">' + GrossAmount + '</td><td style="text-align:right">' + NetAmount + '</td><td class="text-center">  <i class="glyphicon glyphicon-minus-sign text-danger delete-row"></i></td><td style="display:none">' + tempItem.SchemeId + '</td><td style="display:none">' + tempItem.InstanceId + '</td></tr>';
            ...
            $('#btnPrint').click(function () {
                ...
                if (id != 0) {
                    ...
                    var url = "/Sales/Prints/" + type + "/" + number + "/Quote?id=" + id + "&location=" + $.cookie('bsl_2');
                    PopupCenter(url, 'SalesQuote', 800, 700);
                }
            ...
            function save(printSave) {
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        ...
                        console.log(data);
                        $.ajax({
                            url: $(hdApiUrl).val() + 'api/SalesQuote/Save',
                            method: 'POST',
                            ...
                            success: function (response) {
                                if (response.Success) {
                                    ...
                                    if (printSave == true) {
                                        var type = JSON.parse($('#hdSettings').val()).TemplateType;
                                        var number = JSON.parse($('#hdSettings').val()).TemplateNumber;
                                        var url = "/Sales/Prints/" + type + "/" + number + "/Quote?id=" + response.Object.Id + "&location=" + $.cookie('bsl_2');
                                        PopupCenter(url, 'salesQuote', 800, 700);
                                    }
                                }
                                else {
            ...
            $('#btnFind').click(function () {
                ...
                function refreshTable(customerID, fromDate, toDate) {
                    $.ajax({
                        url: $('#hdApiUrl').val() + 'api/SalesQuote/Get?CustomerId=' + customerID + '&from=' + fromDate + '&to=' + toDate,
                        method: 'POST',
                        dataType: 'JSON',
            ...
            $(document).on('click', '#btnSend', function (e) {
                $('.before-send').fadeOut('slow', function () {
                    ...
                    var number = JSON.parse($('#hdSettings').val()).TemplateNumber;
                    $.ajax({
                        url: $('#hdApiUrl').val() + 'api/SalesQuote/SendMail?salesId=' + id + '&toaddress=' + toaddr + '&userid=' + $.cookie('bsl_3'),
                        method: 'POST',
                        datatype: 'JSON',
                        contentType: 'application/json;charset=utf-8',
                        data: JSON.stringify($(location).attr('protocol') + '//' + $(location).attr('host') + '/Sales/Prints/' + type + '/' + number + '/Quote?id=' + id + '&location=' + $.cookie('bsl_2') + '&lid=' + modifiedBy),
                        success: function (response) {
                            if (response.Success) {
                                successAlert(response.Message);
                ...
                if ($('#hdId').val() != 0) {
                    swal({
                        ...
                        if (isConfirm) {
                            ...
                            if (id != 0) {
                                $.ajax({
                                    url: $('#hdApiUrl').val() + 'api/SalesQuote/Confirm/' + id,
                                    method: 'POST',
                                    datatype: 'JSON',
                                    contentType: 'application/json;charset=utf-8',
            ...
            $('#btnDelete').click(function () {
                if ($('#hdId').val() != 0) {
                    ...
                    function (isConfirm) {
                        ...
                        if (isConfirm) {
                            $.ajax({
                                url: $('#hdApiUrl').val() + 'api/SalesQuote/delete/' + id,
                                method: 'DELETE',
                                datatype: 'JSON',
                                contentType: 'application/json;charset=utf-8',
                                data: JSON.stringify(modifiedBy),
