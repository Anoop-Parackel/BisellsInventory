<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="DeliveryNote.aspx.cs" Inherits="BisellsERP.Sales.DeliveryNote" ValidateRequest="false" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <title>Sales DeliveryNote</title>
    <style>
        #wrapper {
            overflow: hidden;
        }
...
<asp:Content ID="Content2" ContentPlaceHolderID="childContent" runat="server">
    ...
    <div class="row p-b-10">
        <div class="col-sm-2">
            <h3 class="page-title m-t-0">Delivery Note</h3>
        </div>
        <div class="col-sm-10">
            <div class="btn-toolbar pull-right" role="group">
                <button type="button" accesskey="v" id="btnFind" data-toggle="tooltip" data-placement="bottom" title="View previous delivery notes" class="btn btn-default waves-effect waves-light"><i class="ion-search"></i></button>
                <button id="btnNew" accesskey="n" type="button" data-toggle="tooltip" data-placement="bottom" title="Start a new delivery note. Unsaved data will be lost" class="btn btn-default waves-effect waves-light"><i class="ion-compose"></i>&nbsp;New</button>
                <button type="button" accesskey="s" id="btnSave" data-toggle="tooltip" data-placement="bottom" title="Save the current delivery note" class="btn btn-default waves-effect waves-light "><i class="ion-archive"></i>&nbsp;Save</button>
                <button id="btnSavePrint" accesskey="a" type="button" data-toggle="tooltip" data-placement="bottom" title="Save & Print the current delivery note" class="btn btn-default waves-effect waves-light"><i class="ion ion-printer"></i>&nbsp;Save & Print</button>
                <button type="button" id="btnConfirm" data-toggle="tooltip" data-placement="bottom" title="Confirm sales delivery note" class="btn btn-default waves-effect waves-light "><i class="ion-checkmark-circled"></i>&nbsp;Confirm</button>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-sm-12">
                                        <%--<asp:TextBox ID="txtAddress" autocomplete="off" ClientIDMode="Static" runat="server" CssClass="form-control input-sm m-t-10" placeholder="Address"></asp:TextBox>--%>
                                         <input type="text" autocomplete="off" class="form-control input-sm m-t-10" id="txtAddress" placeholder="Contact" />
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Previous Delivery Note &nbsp;<a id="findFilter"><i class="fa fa-align-justify"></i></a></h4>
    ...
    <script>
        ...
        $(document).ready(function () {
          ...
          .on('changeDate', function (ev) {
            ...
            //Make visible the hidden address fields
            $('.change-address').click(function () {
                $('.address-tab').removeClass('hidden');
            });

            //To Change billing address by clicking on the li
                ...
                if (tempItem.ItemID != '' & tempItem.ItemID != null & tempItem.ItemID != undefined & qty != '0' & qty != '' & qty != null & !isNaN(qty)) {
                    ...
                    if ((!IsNegBillingAllowed && parseFloat(tempItem.Stock) < qty) && TrackInventory == 'true') {//New Function with track inventory check
                        ...
                        if (itemExists) {
                            ...
                            html = '';
                            html += '<tr><td style="display:none">' + tempItem.ItemID + '</td><td style="display:none">0</td><td><b>' + tempItem.Name + '</b><br/><i contenteditable="true" spellcheck="false">' + description + '</i></td><td>' + tempItem.ItemCode + '</td><td style="text-align:right">' + tempItem.TaxPercentage + '</td><td style="text-align:right">' + tempItem.MRP + '</td><td contenteditable="true" class="numberonly" style="text-align:right">' + tempItem.SellingPrice + '</td><td class="hidden" style="text-align:right">0</td><td style="text-align:right" contenteditable="true" class="numberonly">  ' + qty + ' </td><td style="text-align:right">' + TaxAmount + '</td><td style="text-align:right">' + GrossAmount + '</td><td style="text-align:right">' + NetAmount + '</td><td class="text-center">  <i class="glyphicon glyphicon-minus-sign text-danger delete-row"></i></td><td style="display:none">' + tempItem.SchemeId + '</td><td style="display:none">' + tempItem.InstanceId + '</td></tr>';
            ...
            $('#btnPrint').click(function () {
                ...
                if (id != 0) {
                    ...
                    var url = "/Sales/Prints/" + type + "/" + number + "/DeliveryNote?id=" + id + "&location=" + $.cookie('bsl_2');
                    PopupCenter(url, 'SalesDeliveryNote', 800, 700);
                }
            ...
            function save(printSave) {
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        ...
                        $.ajax({
                            url: $(hdApiUrl).val() + 'api/SalesDeliveryNote/Save',
                            method: 'POST',
                            ...
                            success: function (response) {
                                if (response.Success) {
                                    ...
                                    if (printSave == true) {
                                        var type = JSON.parse($('#hdSettings').val()).TemplateType;
                                        var number = JSON.parse($('#hdSettings').val()).TemplateNumber;
                                        var url = "/Sales/Prints/" + type + "/" + number + "/DeliveryNote?id=" + response.Object.Id + "&location=" + $.cookie('bsl_2');
                                        PopupCenter(url, 'SalesDeliveryNote', 800, 700);
                                    }
                                }
                                else {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            $(response).each(function (index) {
                                html += '<tr>';
                                html += '<td style="display:none">' + this.ID + '</td>';
                                html += '<td>' + this.DeliveryNoteNumber + '</td>';
                                html += '<td>' + this.EntryDateString + '</td>';
                                html += '<td>' + this.CustomerName + '</td>';
                                html += '<td>' + this.TaxAmount + '</td>';
                                html += '<td>' + this.Gross + '</td>';
                        ...
                        success: function (response) {
                            try {
                                ...
                                $(register.Products).each(function () {
                                    ...
                                    html += '<td style="text-align:right" class="hidden">' + this.Quantity + '</td>';
                                    html += '<td style="text-align:right" contenteditable="true">' + this.Quantity + '</td>';
                                    ...
                                    html += '<td class="text-center"> <i class="glyphicon glyphicon-minus-sign text-danger delete-row"></i></td>';
                                    html += '<td style="display:none">' + this.SchemeId + '</td>';
                                    html += '<td style="display:none">' + this.InstanceId + '</td>';
                                    html += '</tr>';
                                });
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            try {
                                ...
                                $('#listTable tbody').append(html);
                                $('#findModal').modal('hide');
                                //alert(register.BillingAddress[0].ContactName);
                                $('#txtContactName').val(register.BillingAddress[0].ContactName);
                                $('#txtPhone1').val(register.BillingAddress[0].Phone1);
                                $('#txtPhone2').val(register.BillingAddress[0].Phone2);
                                $('#ddlSalutation').val(register.BillingAddress[0].Salutation);
                    ...
                    success: function (response) {
                        try {
                            ...
                            $(register.Products).each(function () {
                                ...
                                html += '<td class="hidden" style="text-align:right" contenteditable="true" class="numberonly">' + this.Quantity + '</td>';
                                html += '<td style="text-align:right" contenteditable="true" class="numberonly">' + this.Quantity + '</td>';
                                html += '<td style="text-align:right">' + this.TaxAmount + '</td>';
                                html += '<td style="text-align:right">' + this.Gross + '</td>';
                                html += '<td style="text-align:right">' + this.NetAmount + '</td>';
                ...
                if ($('#hdId').val() != 0) {
                    swal({
                        ...
                        if (isConfirm) {
                            ...
                            if (id != 0) {
                                $.ajax({
                                    url: $('#hdApiUrl').val() + 'api/SalesDeliveryNote/Confirm/' + id,
                                    method: 'POST',
                                    datatype: 'JSON',
                                    contentType: 'application/json;charset=utf-8',
