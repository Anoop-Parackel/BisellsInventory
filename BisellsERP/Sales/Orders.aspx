<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Orders.aspx.cs" Inherits="BisellsERP.Sales.Orders" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="childContent" runat="server">
    ...
    <div class="row p-b-10">
        <div class="col-sm-4">
            <h3 class="page-title m-t-0"> Sales Orders</h3>
        </div>
        <div class="col-sm-8">
                    ...
                    <div class="list-group-item active m-b-10">
                        <div class="btn-group">
                            <button type="button" class="trans-btn dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                                All Sales Orders&nbsp;<b id="countOfInvoices">(0)</b>&nbsp;<span class="caret"></span>
                            </button>
                        ...
                        <input type="text" class="form-control listing-search-entity" placeholder="Seach order number..."/>
                            ...
                            <div class="panel-body">
                                ...
                                <div class="empty-state-wrap" style="display: none">
                                    ...
                                    <div class="btn-group empty-state-buttons">
                                        <a href="/Sales/Quote?MODE=new" class="btn btn-default btn-xs btn-rounded waves-effect">Create New Order</a>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="right-sidebar" style="display: none">
            <div class="col-md-8">
                        ...
                        <div class="btn-toolbar m-t-5" role="toolbar">
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li class="convert-to-delivery"><a href="#">Convert to Delivery Note</a></li>
                                </ul>
                            </div>
                ...
                <div class="panel panel-default m-t-15 right-side">
                    <div class="ribbon success" style="display:none" id="ApproveStatus"><i class="md  md-verified-user"></i>&nbsp;<span id="ApproveStatusText"></span></div>
                    <div class="panel-body">
    ...
    <script>
        ...
        $(document).ready(function ()
        {
            ...
            $.ajax({
                url: $('#hdApiUrl').val() + 'api/SalesQuote/GetCompanyDetails/',
                method: 'POST',
            ...
            //Function for loading list of orders
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        switch (data.ApprovedStatus) {
                            ...
                            default:
                                $('#ApproveStatus').hide();
                                $('#ApproveStatusText').text("");
                                $('.recordPayment-register').addClass('hidden');
                        }
                        if (data.ApprovedStatus==0) {
                            $('.convert-to-invoice').hide();
                            $('.convert-to-delivery').hide();
                        }
                        else {
                            $('.convert-to-invoice').show();
                            $('.convert-to-delivery').show();
                        }
                        $('#hdRegisterId').val(data.ID);
                        $('#hdEmail').val(data.BillingAddress[0].Email);
                        $('#lblCustName').text(data.Customer_Name);
            ...
            $(document).on('click', '.edit-register', function () {
                window.open('/Sales/Quote?MODE=edit&UID=' + $('#hdRegisterId').val(), '_self');
            });

            //Convert to invoice Click event
            $(document).on('click', '.convert-to-invoice', function () {
                window.open('/Sales/Entry?MODE=convert&TYPE=SQT&UID=' + $('#hdRegisterId').val(), '_self');
            });

            //Convert to Delivery Click event
            $(document).on('click', '.convert-to-delivery', function () {
                window.open('/Sales/DeliveryNote?MODE=convert&TYPE=SQT&UID=' + $('#hdRegisterId').val(), '_self');
            });
            ...
            $(document).on('click', '.print-register', function () {
                ...
                PopupCenter('/Sales/Prints/' + type + '/' + number + '/Quote?id=' + $('#hdRegisterId').val() + '&location=' + $.cookie('bsl_2'), 'salesOrders', 800, 700);
            });
            $(document).on('click', '.clone-register', function () {
                window.open('/Sales/Quote?MODE=clone&UID=' + $('#hdRegisterId').val(), '_self');
            });
            $(document).on('click', '.new-register', function () {
                window.open('/Sales/Quote?MODE=new', '_self');
            });
