<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="CreditNote.aspx.cs" Inherits="BisellsERP.Sales.CreditNote" ValidateRequest="false" %>
    ...
    <div class="row p-b-10">
        ...
        <div class="col-sm-10">
            <div class="btn-toolbar pull-right" role="group">
                ...
                <%-- <button type="button" id="btnConfirm" disabled="disabled" data-toggle="tooltip" data-placement="bottom" title="Confirm CreditNote" class="btn btn-default waves-effect waves-light "><i class="ion-checkmark-circled"></i>&nbsp;Confirm</button>--%>
                            ...
                            <div class="portlet-body">
                                <div class="row">
                                    <div class="col-sm-6">
                                        ...
                                        <input type="text" autocomplete="off" class="form-control" id="txtName" placeholder="Name" />
                                    </div>
                                    <div class="col-sm-6">
        ...
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Previous Credit Note &nbsp;<a id="findFilter"><i class="fa fa-align-justify"></i></a></h4>
          ...
          .on('changeDate', function (ev) {
                    ...
                    if ((!IsNegBillingAllowed && parseFloat(tempItem.Stock) < qty) && TrackInventory == 'true') {//New Function with track inventory check
                        ...
                        if (itemExists) {
                            ...
                            if (parseFloat(tempItem.Stock) < newQty && TrackInventory == 'true') {
                                ...
                                $(rowOfItem).children('td').eq(8).replaceWith('<td style="text-align:right;">' + newQty + '</td>');
                            ...
                            html = '';
                            html += '<tr><td style="display:none">' + tempItem.ItemID + '</td><td style="display:none">0</td><td><b>' + tempItem.Name + '</b><br/></td><td>' + tempItem.ItemCode + '</td><td style="text-align:right">' + tempItem.TaxPercentage + '</td><td style="text-align:right">' + tempItem.MRP + '</td><td contenteditable="true" class="numberonly" style="text-align:right">' + tempItem.SellingPrice + '</td><td class="hidden" style="text-align:right">0</td><td style="text-align:right">  ' + qty + ' </td><td style="text-align:right">' + TaxAmount + '</td><td style="text-align:right">' + GrossAmount + '</td><td style="text-align:right">' + NetAmount + '</td><td class="text-center">  <i class="glyphicon glyphicon-minus-sign text-danger delete-row"></i></td><td style="display:none">' + tempItem.SchemeId + '</td><td style="display:none">' + tempItem.InstanceId + '</td></tr>';
            ...
            $('#btnPrint').click(function () {
                ...
                if (id != 0) {
                    ...
                    PopupCenter(url, 'SalesReturn', 800, 700);
                }
            ...
            function save(printSave) {
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        ...
                        data.ReturnDateString = entryDate;
                        ...
                        console.log(data);
                        $.ajax({
                            url: $(hdApiUrl).val() + 'api/CreditNote/Save',
                            method: 'POST',
                            ...
                            success: function (response) {
                                if (response.Success) {
                                    ...
                                    if (printSave == true) {
                                        var type = JSON.parse($('#hdSettings').val()).TemplateType;
                                        var number = JSON.parse($('#hdSettings').val()).TemplateNumber;
                                        var url = "/Sales/Prints/" + type + "/" + number + "/Return?id=" + response.Object.Id + "&location=" + $.cookie('bsl_2');
                                        PopupCenter(url, 'SalesReturn', 800, 700);
                                    }
                                }
                                else {
            ...
            function getRegister(isClone, id) {
                resetRegister();
                $.ajax({
                    url: $('#hdApiUrl').val() + 'api/CreditNote/get/' + id,
                    method: 'POST',
                    dataType: 'JSON',
                    ...
                    success: function (response) {
                        try {
                            ...
                            $('#lblOrderNumber').text(register.CreditNoteNumber);
            ...
            $('#btnFind').click(function () {
                ...
                function refreshTable(customerID, fromDate, toDate) {
                    $.ajax({
                        url: $('#hdApiUrl').val() + 'api/CreditNote/Get?CustomerId=' + customerID + '&from=' + fromDate + '&to=' + toDate,
                        method: 'POST',
                        dataType: 'JSON',
                        data: JSON.stringify($.cookie('bsl_2')),
                        contentType: 'application/json;charset=utf-8',
            ...
            $(document).on('click', '#btnSend', function (e) {
                $('.before-send').fadeOut('slow', function () {
                    ...
                    var number = JSON.parse($('#hdSettings').val()).TemplateNumber;
                    $.ajax({
                        url: $('#hdApiUrl').val() + 'api/CreditNote/SendMail?salesId=' + id + '&toaddress=' + toaddr + '&userid=' + $.cookie('bsl_3'),
                        method: 'POST',
                        datatype: 'JSON',
                        contentType: 'application/json;charset=utf-8',
                ...
                if ($('#hdId').val() != 0) {
                    swal({
                        ...
                        if (isConfirm) {
                            ...
                            if (id != 0) {
                                $.ajax({
                                    url: $('#hdApiUrl').val() + 'api/CreditNote/Confirm/' + id,
                                    method: 'POST',
                                    datatype: 'JSON',
                                    contentType: 'application/json;charset=utf-8',
            ...
            $('#btnDelete').click(function () {
                if ($('#hdId').val() != 0) {
                    ...
                    function (isConfirm) {
                        ...
                        if (isConfirm) {
                            $.ajax({
                                url: $('#hdApiUrl').val() + 'api/CreditNote/delete/' + id,
                                method: 'DELETE',
                                datatype: 'JSON',
                                contentType: 'application/json;charset=utf-8',
                                data: JSON.stringify(modifiedBy),
