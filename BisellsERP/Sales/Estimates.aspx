<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Estimates.aspx.cs" Inherits="BisellsERP.Sales.Estimates" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="childContent" runat="server">
    ...
    <div class="row p-b-10">
        <div class="col-sm-4">
            <h3 class="page-title m-t-0">Estimates</h3>
        </div>
        <div class="col-sm-8">
                    ...
                    <div class="list-group-item active m-b-10">
                        <div class="btn-group">
                            <button type="button" class="trans-btn dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                                All Estimates&nbsp;<b id="countOfInvoices">(0)</b>&nbsp;<span class="caret"></span>
                            </button>
                         ...
                         <input type="text" class="form-control listing-search-entity" placeholder="Search estimate number..."/>
                            ...
                            <div class="panel-body">
                                <table id="invoiceList" class="table left-table invoice-list">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="show-on-collapsed">Estimate Number</th>
                                ...
                                <div class="empty-state-wrap" style="display: none">
                                    ...
                                    <div class="btn-group empty-state-buttons">
                                        <a href="/Sales/Quote?MODE=new" class="btn btn-default btn-xs btn-rounded waves-effect">Create New Estimate</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="btn-toolbar m-t-5" role="toolbar">
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <li><a id="btnApproval" href="#">Approve Estimate</a></li>
                                    <li class="divider"></li>
                                    <li class="convert-to-invoice"><a href="#">Convert to Invoice</a></li>
                                    <li class="convert-to-order"><a href="#">Convert to Sales Order</a></li>
                                ...
                                <div class="">
                                    ...
                                    <div class="row" style="margin-top: 15px;">
                                        ...
                                        <div class="col-xs-4 text-right invoice-info">
                                            ...
                                            <p class="m-0">
                                                <span>Estimate No<span class="pull-right">:</span></span>
                                                <b id="lblInvoiceNo"></b>
                                            </p>
                                            <p class="m-0">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $.ajax({
                url: $('#hdApiUrl').val() + 'api/SalesEstimate/GetCompanyDetails/',
                method: 'POST',
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $(data).each(function (index) {
                            ...
                            html += '<td class="show-on-collapsed">' + this.EstimateNumber + '</td>';
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        switch (data.ApprovedStatus) {
                            case 0:
                                $('#btnApproval').html('Approve Estimate');
                        ...
                        console.log(data.BillingAddress[0].Email)
                        ...
                        $('#lblInvoiceNo').text(data.EstimateNumber);
            ...
            $('#btnApproval').click(function () {
                ...
                $.ajax
                    ({
                        url: $('#hdApiUrl').val() + 'api/SalesEstimate/ToggleConfirm/' + id,
                        method: 'POST',
                        contentType: 'application/json; charset=utf-8',
                        dataType: 'Json',
            ...
            $(document).on('click', '.edit-register', function () {
                window.open('/Sales/Estimate?MODE=edit&UID=' + $('#hdRegisterId').val(), '_self');
            });
            $(document).on('click', '.print-register', function () {
                ...
                PopupCenter('/Sales/Prints/' + type + '/' + number + '/Estimate?id=' + $('#hdRegisterId').val() + '&location=' + $.cookie('bsl_2'), 'salesEstimates', 800, 700);
            });
            $(document).on('click', '.clone-register', function () {
                window.open('/Sales/Estimate?MODE=clone&UID=' + $('#hdRegisterId').val(), '_self');
            });
            //Convert to invoice Click event
            $(document).on('click', '.convert-to-invoice', function () {
                window.open('/Sales/Entry?MODE=convert&TYPE=EST&UID=' + $('#hdRegisterId').val(), '_self');
            });
            //Convert to Order click event
            $(document).on('click', '.convert-to-order', function () {
                window.open('/Sales/Quote?MODE=convert&TYPE=EST&UID=' + $('#hdRegisterId').val(), '_self');
            });
            $(document).on('click', '.convert-to-delivery', function () {
                window.open('/Sales/DeliveryNote?MODE=convert&TYPE=EST&UID=' + $('#hdRegisterId').val(), '_self');
            });

            $(document).on('click', '.new-register', function () {
                window.open('/Sales/Estimate?MODE=new', '_self');
            });
