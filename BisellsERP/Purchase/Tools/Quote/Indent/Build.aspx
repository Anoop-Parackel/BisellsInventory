<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Build.aspx.cs" Inherits="BisellsERP.Purchase.Tools.Indent.Build" %>
                        ...
                        <div class="modal-title">
                            <div class="row">
                                <div class="col-sm-4 col-md-3">
                                    <span>Location:
                                        <label id="lblLocation" class="badge badge-danger pw-15 hidden">xx</label>
                                    </span>
                                </div>
    ...
    <script>
        ...
        $(document).ready(function ()
        {
            ...
            $('#btnLoad').off().click(function ()
            {
                ...
                $.ajax({
                    url: $('#hdApiUrl').val() + '/api/quotebuilder/GetFromIndent',
                    method: 'POST',
                    ...
                    success: function (response) {
                        ...
                        for (var i = 0; i < response.length; i++)
                        {
                            ...
                            html += '<td>' + response[i].IndentNo + '</td>';
                        ...
                        $('#listTable').off().on('click', '.build-quote', function ()
                        {
                            ...
                            var locationid = 1;
                            $.ajax({
                                url: $('#hdApiUrl').val() + '/api/QuoteBuilder/GetIndent/?RequestId=' + reqId,
                                method: 'POST',
                                            ...
                                            for (var i = 0; i < requestCarts.Qoutes.length; i++)
                                            {
                                                ...
                                                if (requestCarts.Qoutes[i].SupplierId === supplierid)
                                                {
                                                    ...
                                                    $(products).each(function (index)
                                                    {
                                                        ...
                                                        html += '<td>' + this.ItemCode + '</td>';
                                                        html += '<td>' + this.Mrp + '</td>';
                                                        html += '<td contenteditable="true">' + this.Rate + '</td>';
            ...
            $('#btnSave').click(function ()
            {
                ...
                function (isConfirm)
                {
                    if (isConfirm)
                    {
                        if (sessionStorage.getItem('requestCarts') != null)
                        {
                            console.log(sessionStorage.getItem('requestCarts'));
                            $.ajax({
                                method: 'POST',
                                url: $('#hdApiUrl').val() + 'api/QuoteBuilder/SaveFromIndent',
                                dataType: 'JSON',
                                contentType: 'application/json;charset=utf-8',
